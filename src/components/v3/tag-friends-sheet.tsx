...
/**
 * F04 "tag friends" (also reused to manage the close-friends list). A Vaul
 * bottom sheet: search + the caller's following with checkmarks, picked people
 * as removable pills, a live count in the done button.
 *
 * Browse-first — the search input does NOT autofocus on open, so the iOS
 * keyboard doesn't fight the drawer's focus scope (the focus-trap risk is the
 * autofocus-on-open case; tapping the field to type is fine, same as the
 * shipped how-was-it-sheet's textarea-in-Vaul).
 *
 * Selection is parent-controlled (value + onChange). `onDone` commits (distinct
 * from `onClose` = cancel). `seedFollowing` lets a caller hand in an already-
 * fetched following list (close-friends manage) so we don't re-read it.
 */
...
export function TagFriendsSheet({
  ...
  const remove = (uid: string) => { haptic('selection'); onChange(value.filter((x) => x.uid !== uid)); };
      ...
      <Drawer.Portal>
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-2.5">
            ...
            <span className="font-headline font-bold text-[19px] lowercase tracking-[-0.02em]">{title}</span>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))]">
            {loading ? (
              ...
              <p className="font-serif italic text-[15px] text-muted-foreground py-8 text-center">
                {query.trim() ? (searching ? 'searching…' : `no one matching "${query.trim()}".`) : 'follow some people first to tag them.'}
              </p>
            ) : (
              <div>
                {list.map((u, i) => {
                  const on = selectedById.has(u.uid);
                  return (
                    <button key={u.uid} onClick={() => toggle(u)} className="relative w-full flex items-center gap-3.5 py-3 text-left active:bg-foreground/[0.03]">
                      <ProfileAvatar photoURL={u.photoURL} displayName={u.displayName} username={u.username} size="md" className="flex-shrink-0" />
                      <span className="flex-1 min-w-0">
                        <span className="block truncate font-headline font-bold text-[16px] lowercase tracking-[-0.02em] text-foreground">{u.displayName || u.username || 'user'}</span>
                      ...
                      {i < list.length - 1 && <div className="absolute bottom-0 left-[56px] right-0 h-px bg-rule" />}
                    </button>
                  );
                })}
              </div>
