...
export function SocialSignInButtons({ onSuccess }: Props) {
  ...
  // Capacitor detection only stabilizes after hydration; the Apple button
  // would otherwise flash on then off in the iOS WebView.
  ...
  return (
    <div className="space-y-3">
      <Button
        type="button"
        variant="outline"
        className="w-full border border-border rounded-full shadow-lift bg-card hover:bg-secondary/50"
      ...
      {showApple && (
        <Button
          type="button"
          className="w-full bg-black text-white hover:bg-black/90 rounded-full shadow-lift"
          onClick={handleApple}
          disabled={busy !== null}
        >
          {busy === 'apple' ? (
            ...
            <AppleGlyph className="h-4 w-4 mr-2" />
          )}
          Sign in with Apple
        </Button>
      )}
