...
- [ ] **2.4.1** Replace the `throw` with a toast + console + (eventually) Sentry. Errors should not bubble through render.
- [ ] **2.4.2** Tighten `non-blocking-updates.tsx:19-89` to only emit `permission-error` for actual permission errors, not all write failures.
- [ ] **2.4.3** **Test (manual):** simulate offline state, attempt a write, then reconnect. App must not crash to `global-error.tsx` — toast appears, write retries or fails silently.
