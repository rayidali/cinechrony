...
| #18 | Phase A.5 — remaining Server Actions | Migrate the ~15 Server Actions left in `actions.ts` (`searchPublicLists`, `getLovedLists`, `getListMembers`, `getListsPreviews`, `getListPreview`, `getUserLists`, `getCollaborativeLists`, `isFollowing`, `checkUsernameAvailability`, `createUserProfileWithUsername`, `ensureUserProfile`, `parseLetterboxdExport`, `parseAndMatchMovies`, `importMatchedMovies`, `importLetterboxdMovies`) so the static export can actually build. | — | pending |
